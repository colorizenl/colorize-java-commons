 ...
 * <p>
 * Exposing a {@link Subject} allows users to both subscribe to events and
 * to publish events. If you only want to allow users to subscribe, so
 * <em>without</em> the ability to publish events themselves, expose the
 * {@link Subject} as a {@link Publisher} or {@link Subscribable}.
 *
 * @param <T> The type of event that can be subscribed to.
 */
public final class Subject<T> implements Subscribable<T> {
    ...
    private void registerSubscription(Subscriber<? super T> subscriber) {
     ...
     * undelivered errors.
     */
    public void subscribeErrors(Consumer<Throwable> onError) {
     ...
     */
    public void subscribe(Subject<T> subscriber) {
